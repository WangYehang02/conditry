#!/usr/bin/env python3
"""Diffusion ablation + polarity (reuse trained EDM checkpoints).

Same as diffusion ablation (GAE + residual + free EDM + AE recon),
but polarity_enabled=true with per-dataset consensus hyperparams.
Still no prototype guidance.
"""
...
CKPT_ROOT = REPO / "results" / "diffusion_ablation"
...
def _run_one(
    ...
    # Keep per-dataset polarity threshold / score_weight from yaml.
    cfg["polarity_enabled"] = True
    cfg["dataset"] = dataset
    # Reuse diffusion ablation checkpoint paths.
    ...
    env["FMGAD_MODEL_ROOT"] = str((CKPT_ROOT / "models").resolve())
    ...
    print(
        f"[diff+polar] {dataset} s{seed} gpu={gpu} rc={rc} auc={auc:.4f} {time.time()-t0:.0f}s",
        flush=True,
    )
...
def _aggregate(out_root: Path) -> Path:
    ...
    # Load no-polar diffusion for side-by-side
    base = {}
    base_path = CKPT_ROOT / "summary" / "diffusion_ablation_means.json"
    if base_path.exists():
        base = json.load(open(base_path)).get("auc", {})

    summary_dir = out_root / "summary"
    summary_dir.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Diffusion + Polarity Ablation",
        "",
        "Reuses Diffusion free-only EDM checkpoints; enables polarity calibration.",
        "Still `use_proto=false`. Seeds `{0,1,2,3,42}`; cells rounded to 3 decimals.",
        "",
    ...
    if base:
        lines.append(
            "| Diffusion (no polar) | "
            + " | ".join(f"{base[d]:.3f}" for d in DATASETS)
            + f" | {base['avg']:.3f} |"
        )
    lines.append(
        "| Diffusion + Polarity | "
        + " | ".join(f"{means[d]:.3f}" if not np.isnan(means[d]) else "--" for d in DATASETS)
        + (f" | {means['avg']:.3f} |" if not np.isnan(means["avg"]) else " | -- |")
    )
    lines.append("")
    md_path = summary_dir / "diffusion_polar_table.md"
    md_path.write_text("\n".join(lines), encoding="utf-8")
    with open(summary_dir / "diffusion_polar_means.json", "w", encoding="utf-8") as f:
        json.dump({"auc": means, "overrides": OVERRIDES}, f, indent=2)
...
def main() -> int:
    ...
    ap.add_argument(
        "--output-root",
        type=str,
        default=str(REPO / "results" / "diffusion_polar_ablation"),
    )
    ...
    print(f"Scheduling {len(jobs)} diffusion+polar jobs on GPUs {gpus}", flush=True)
    ...
    _aggregate(out_root)
    print(f"Done. fails={fails}. Summary: {out_root / 'summary' / 'diffusion_polar_table.md'}", flush=True)
